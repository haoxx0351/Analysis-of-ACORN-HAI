...
# Load data
df <- readRDS("data/clean_data_RData/baseline_outcomes_index.RData")
epi <- readRDS("data/clean_data_RData/vap_bsi_index.RData")
...
df_add <- df_add %>%
  mutate(
    GN_car_r = case_when(
      ...
      ris_Carbapenems ==5 | Class != "GNB" ~ NA_real_,
      TRUE ~ 0
    ),
    GN_thir_r = case_when(
      `ris_Third-generation cephalosporins` == 1 & ris_Carbapenems %in% c(2,3) & Class == "GNB" ~ 1,
      `ris_Third-generation cephalosporins` == 5 | Class != "GNB" ~ NA_real_,
      TRUE ~ 0
    )
  ) %>% 
  ...
  select (recordid, infection_types, pathogen_combined_types, GN_car_thir) %>% 
  group_by(recordid) %>%
  filter(!is.na(GN_car_thir) | row_number() == 1) %>%
  ungroup()
...
saveRDS(df, "data/clean_data_RData/data_table_index_new.RData")

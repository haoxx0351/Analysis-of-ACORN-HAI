...
# Plot
df_plot <- df[which(df$infection_types %in% "Hospital-acquired BSI"),]
...
p <- ggplot() +
  ...
  theme_bw() + 
  labs(x = "", y = "", title = "AST results (hospital-acquired BSI)",
...
# Save
CairoPDF("output/figure/pie_charts_ast_bsi_hosp.pdf", width = 12, height = 10.5)
print(p)
dev.off()
###
...
# Load data
df <- readRDS("data/clean data_V2/readmission60_data.RData")
...
# Save
saveRDS(df, "data/clean data_V2/readmission_table_index.RData")
...
fig <- forestplot(
  ...
  hrzl_lines = list(               
    ...
    "2" = gpar(lty = 2),           
    "45" = gpar(lwd = 1.5, lty = 1, columns = c(1:3)) 
  ),
  graphwidth = unit(.25, "npc"),   
  xlab = "Hazard ratio (HR)", 
  xticks = c(seq(0, 6, 0.5)), 
  ...
  title = "Univariable analysis (stratified by infection types)",   
  col = fpColors(               
    box = "blue4",                
    lines = "blue4",               
    zero = "black"                 
...
# Save
pdf(file = "output/figure/readmission_crudeHR.pdf", width = 13, height = 11)
print(fig)
dev.off()

# Save table
result_save <- result[, c(1, 5, 6)]
saveRDS(result_save, "data/clean data_V2/readmission_table_univariable.RData")
###
...
gg_cuminc <- cuminc |>
  ...
  theme(
    ...
    legend.title = element_text(size = 10, color = "black", face = "bold", family = "Times New Roman")
  ) +
  ...
  annotate("text", x = 2.9, y = 0.3, label = "p < 0.001 (all-cause death)",
...
gg_risktable <- 
  ...
  mutate(outcome = recode(outcome, 
                          ...
                          "Readmission" = "Readm", 
                          "At Risk" = "At risk")) |> 
  mutate(outcome = factor(outcome, levels = c("death", "Readm", "At risk"))) |>
...
combined_plot <- wrap_plots(
  ...
  plot_annotation(
    theme = theme(plot.margin = margin(10, 10, 10, 35))
  )

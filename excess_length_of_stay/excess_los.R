...
car_ent <- organism %>%
  filter(org_names_all %in% c("E.coli", "Klebsiella pneumoniae", "Klebsiella", "Enterobacter", "Serratia", "Proteus", "Morganella") & ris_Carbapenems %in% 1:3) %>%
  group_by(recordid) %>%
  arrange(ris_Carbapenems) %>%
  slice(1) %>%
  ungroup() %>%
...
thir_ceph_ent <- organism %>%
  filter(org_names_all %in% c("E.coli", "Klebsiella pneumoniae", "Klebsiella", "Enterobacter", "Serratia", "Proteus", "Morganella") & `ris_Third-generation cephalosporins` %in% 1:3) %>%
  group_by(recordid) %>%
...
results_combined <- data.frame(
  pathogen = c("Carbapenem-resistant Acinetobacter",
...
# Save
gtsave(data = table, filename = "excess_LOS.html")

...
for (i in seq_along(df_organism)) {
  ...
  pop_p <- length(which((df_organism[[i]]$ris_Carbapenems == 1) & (df_organism[[i]]$org_names_all == "Acinetobacter"))) / nrow(df_organism[[i]])
...
# Define model scales
model_scales <- c("VAP" = 1.478, "Hospital-acquired BSI" = 1.630, "Healthcare-associated BSI" = 2.170)
...
# Save
saveRDS(ar_af_carbapenems, "data/att28_table_paf_01.RData")

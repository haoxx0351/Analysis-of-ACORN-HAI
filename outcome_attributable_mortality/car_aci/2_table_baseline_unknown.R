...
label(df$ris_Carbapenems) <- "Acinetobacter spp."
...
# Modify the label
label(df$pathogen_combined_types) <- "Type of pathogens"
...
table <- df %>%
  select(age_new, sex, 
         ...
         icu_hd_ap, 
         pathogen_combined_types, ris_Carbapenems,

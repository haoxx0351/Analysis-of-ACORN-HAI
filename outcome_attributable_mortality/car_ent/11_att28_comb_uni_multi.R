...
table <- res %>%
  ...
  tab_style(
    style = cell_text(align = "left", v_align = "middle", font = "Poppins"),
  ...
  tab_source_note(
    source_note = "Note: [1] stratified by infection syndromes, [2] final model."
  ) %>%
  tab_source_note(
    source_note = "Abbreviations: HR = Hazard Ratio, CI = Confidence Interval."
  ) %>%
  tab_style(
    style = cell_text(weight = "bold", font = "Poppins"),
    locations = cells_body(

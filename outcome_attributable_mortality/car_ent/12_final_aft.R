...
for (i in seq_along(df_organism)) {
  ...
  pop_p <- length(which((df_organism[[i]]$ris_Carbapenems == 1) & (df_organism[[i]]$org_names_all %in% c("E. coli", "K. pneumoniae", "Klebsiella", "Enterobacter", "Serratia", "Proteus", "Morganella")))) / nrow(df_organism[[i]])
...
# Define model scales
model_scales <- c("VAP" = 1.807, "Hospital-acquired BSI" = 1.963, "Healthcare-associated BSI" = 1.863)

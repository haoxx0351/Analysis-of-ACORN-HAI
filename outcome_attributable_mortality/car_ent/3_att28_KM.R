...
tbl_fct <- 
  ...
  geom_text(aes(label = n.risk), size = 4.5, family = "Poppins") +
  ...
  theme(panel.grid.major = element_line(color = "gray90", linewidth = 0.5), 
        ...
        axis.text.y = element_text(color="black", size = 12, family = "Poppins", margin = margin(r = 8)),
        axis.text.x = element_blank(),
        strip.text = element_blank(),
        legend.position = "none",
        panel.spacing = unit(0.6, "cm"),
        axis.ticks = element_line(color = "black", linewidth = 0), 
        axis.ticks.length = unit(0, "cm")) +
  scale_y_discrete(limits = c("0", "1"),
                   labels = c("1" = "CRE", "0" = "CSE")) +
  scale_x_continuous(
    limits = c(0, 28), 
...
plt_fct <- ggsurvplot_facet(
  ...
  conf.int = TRUE,
  linetype = 1,
  ggtheme = theme_minimal(base_family = "Poppins") +
    theme(
  ...
  panel.labs = list(infection_types = c("VAP", 
                                        "Hospital-acquired BSI", 
                                        "Healthcare-associated BSI")), 
  panel.labs.font = list(size = 12, family = "Poppins"),
  short.panel.labs = TRUE,
  legend.title = " ",
  legend.labs = c("Carbapenem-susceptible Enterobacterales",
...
# Combine plots with Poppins font
combined_plot <- (plt_fct + theme(plot.margin = margin(t = 2, r = 1, b = 2, l = 1))) /
  (tbl_fct + theme(plot.margin = margin(t = 0, r = 1, b = 2, l = 1))) +
  plot_layout(heights = c(7.5, 2.5)) & theme(plot.margin = margin(t = 0, r = 5, b = 0, l = 5))

# Display the combined plot
print(combined_plot)

# Print the Kaplan-Meier plot
CairoPDF(file = "output/figure/att28_death_KM_03.pdf", width = 15, height = 6)
print(combined_plot)
dev.off()
###
...
for (i in seq_along(df_organism)) {
  ...
  pop_p <- length(which((df_organism[[i]]$`ris_Third-generation cephalosporins` == 1) & (df_organism[[i]]$org_names_all %in% c("E. coli", "K. pneumoniae", "Klebsiella", "Enterobacter", "Serratia", "Proteus", "Morganella")))) / nrow(df_organism[[i]])

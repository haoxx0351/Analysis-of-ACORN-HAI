...
table_row <- list(
  ...
  c("Pulmonary disease", vif_df$vif_values[12]),
  c("Renal disorder", vif_df$vif_values[13]),
  c("Trauma", vif_df$vif_values[14]),
  c("Others", vif_df$vif_values[15]),
  ...
  c("Third-generation cephalosporins-resistant", vif_df$vif_values[18]),
  c("Carbapenem-resistant", vif_df$vif_values[19])
)
...
table_gt <- table_vif %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = 1,
      rows = c(1, 3, 6, 10, 15, 26, 28, 30, 33, 36) 
    )
  ) %>%
  tab_style(
    style = cell_text(

...
aft_model <- survreg(formula = Surv(time, event) ~ 
                    age_new + sex + 
                    country_income +
                    hpd_admreason + 
...
predicted_survival_time <- predict(aft_model, newdata = df, type = "response")

# Define a function to compute survival probabilities
survival_function <- function(t, model, newdata) {
  exp(-exp(predict(model, newdata = newdata, type = "link")) * t)
}

# Generate a sequence of times for plotting
times <- seq(0, max(predicted_survival_time), length.out = 100)

# Compute survival probabilities for each time point
survival_curve <- sapply(times, function(t) survival_function(t, aft_model, df))

# Create a data frame for plotting
survival_data <- data.frame(
  time = rep(times, each = nrow(df)),
  survival = as.vector(t(survival_curve))
)

# Plot the survival curve
ggplot(survival_data, aes(x = time, y = survival)) +
  geom_line() +
  labs(title = "Survival Curve from AFT Model", x = "Time", y = "Survival Probability") +
  theme_minimal()

predicted_survival <- predict(model, newdata = df, type = "response")



# Fit a Kaplan-Meier model
fit <- survfit(Surv(time, event) ~ infection_types + ris_Carbapenems, data = df)
...
# Risk table
tbl_fct <- 
  ggplot(ggsurv$table$data, aes(time, ris_Carbapenems, color = ris_Carbapenems)) + 
...
plt_fct <- ggsurvplot_facet(
  ...
  pval = FALSE,  # Turn off the default p-values
  ...
  legend.title = " ",
  legend.labs = c("Carbapenem-susceptible Acinetobacter",
...
pdf(file = "output/att28_death_KM_01.pdf", width = 13, height = 6, onefile = FALSE)
print(combined_plot)
dev.off()
###